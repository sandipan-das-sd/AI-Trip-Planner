...
import { Link } from 'react-router-dom';
import { GetPhotoUrl } from '@/service/GlobalApi';
import { FaMapMarkerAlt, FaStar, FaHeart, FaRegHeart, FaRupeeSign } from 'react-icons/fa';

// Hotel fallback images by category
const HOTEL_FALLBACKS = {
  luxury: "https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=600&auto=format&fit=crop",
  midrange: "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&auto=format&fit=crop",
  budget: "https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=600&auto=format&fit=crop",
  default: "https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=600&auto=format&fit=crop"
};
      ...
      <Link to={`https://www.google.com/maps/search/?api=1&query=${hotelQuery}`} target='_blank'>
        <div className='bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100 h-full flex flex-col'>
          ...
          <div className='p-4 flex flex-col flex-grow'>
            <h2 className='font-medium text-lg mb-1 line-clamp-1'>{hotel?.name}</h2>
            
            {hotel?.address && (
              <div className="flex items-start mb-2">
                <FaMapMarkerAlt className="text-gray-400 mt-1 mr-1 flex-shrink-0" size={12} />
            ...
            <div className="mt-auto pt-3 border-t border-gray-100">
              <p className='text-sm font-medium flex items-center'>
                ...
                <span className="text-green-600">{getMainPrice(hotel?.price)}</span>
                <span className='text-xs text-gray-500 ml-1'>onwards</span>
              </p>
              <p className='text-xs text-gray-500'>{formatPrice(hotel?.price)}</p>
            </div>
          </div>
